...
@Component({
  ...
  template: ` <div class="page-title">
      <div>
        <h1>My Results</h1>
        <p>Only your own submissions are visible here.</p>
      </div>
    </div>
    <div class="grid grid-2">
      <div class="card">
        ...
        <div *ngIf="!results.length" class="empty">
          <h3>No results</h3>
          <p>Complete an assigned test to create your first result.</p>
        </div>
      </div>
      <div class="card" *ngIf="selected">
        <div class="page-title" style="margin-bottom:16px">
          <div>
            <h2 style="font-size:18px;margin:0">{{ selected.testTitle }}</h2>
            <p>Submitted {{ date(selected.submittedAt) }}</p>
          </div>
        ...
        <div class="mt">
          ...
          >
            ...
            <div class="choice-grid">
              <div *ngFor="let c of a.choices; let j = index" class="choice">
                <span>{{ letters[j] }}.</span><span style="flex:1">{{ c }}</span
                ><span
                  class="answer-chip correct"
                  *ngIf="j === a.correctAnswerIndex"
                  >Correct</span
                ><span
                  class="answer-chip wrong"
                  *ngIf="
...
export class StudentResultsComponent {
  ...
  ngOnInit() {
    this.api.results().subscribe((r) => {
      this.results = r;
      const id = Number(this.route.snapshot.queryParamMap.get("open"));
      if (id) this.open(id);
    });
  }
  open(id: number) {
